...
let s:attr_none   = " term=NONE"          ." gui=NONE"          . " cterm=NONE"
let s:attr_bold   = " term=bold"          ." gui=bold"          . " cterm=bold"
let s:attr_italic = " term=italic"        ." gui=italic"        . " cterm=italic"
let s:attr_underl = " term=underline"     ." gui=underline"     . " cterm=underline"
let s:attr_undero = " term=undercurl"     ." gui=undercurl"     . " cterm=undercurl"
let s:attr_rever  = " term=reverse"       ." gui=reverse"       . " cterm=reverse"
let s:attr_strik  = " term=strikethrough" ." gui=strikethrough" . " cterm=strikethrough"
let s:attr_reverb = " term=reverse,bold"  ." gui=reverse,bold"  . " cterm=reverse,bold"
let s:attr_underlb= " term=underline,bold"." gui=underline,bold". " cterm=underline,bold"
...
" c/c++
exe "hi! cIncluded"             . s:attr_none   . s:fg_cyan    . s:bg_none
exe "hi! link cStructKeyword NeosolarizedKeyword"
exe "hi! link cStructure cStructKeyword"
exe "hi! link cppStructure cStructKeyword"

exe "hi! cTypeHl"               . s:attr_none   . s:fg_orange  . s:bg_none
exe "hi! link cType cTypeHl"
exe "hi! link cppType cTypeHl"

exe "hi! link cppAccess NeosolarizedKeyword"
exe "hi! link cppModifier NeosolarizedKeyword"
exe "hi! link cStorageClass NeosolarizedKeyword"

exe "hi! cppConstant"           . s:attr_none   . s:fg_yellow  . s:bg_none
exe "hi! cppBoolean"            . s:attr_none   . s:fg_yellow  . s:bg_none

" cs
exe "hi! csType"                . s:attr_none   . s:fg_green   . s:bg_none
exe "hi! csModifier"            . s:attr_bold   . s:fg_green   . s:bg_none
exe "hi! csStorage"             . s:attr_bold   . s:fg_green   . s:bg_none

" fennel
exe "hi! link fennelDelimiter NONE"
exe "hi! link fennelLuaTableItemAccessor NONE"
exe "hi! fennelKeyword"         . s:attr_none   . s:fg_cyan    . s:bg_none

" }}}

" Tree-sitter & LSP highlightings {{{
" ------------------------------------------------
if has('nvim')
        " common
	...
	exe "hi! javaScript.ext.SecondaryKeyword"
        ...
        " cs
        exe "hi! cs.ext.Type"
                        \ . s:attr_none   . s:fg_orange  . s:bg_none
        exe "hi! link @lsp.type.class.cs cs.ext.Type"
        exe "hi! link @lsp.type.enum.cs cs.ext.Type"
        exe "hi! link @lsp.type.interface.cs cs.ext.Type"
        exe "hi! link @lsp.type.struct.cs cs.ext.Type"
        exe "hi! link @lsp.type.namespace.cs cs.ext.Type"
        exe "hi! link @lsp.type.keyword.cs csType"

        " c/c++
        exe "hi! link @lsp.type.class.cpp cTypeHl"
        exe "hi! link @lsp.type.enum.cpp cTypeHl"
        exe "hi! link @lsp.type.namespace.cpp cTypeHl"
        exe "hi! link @lsp.type.type.cpp cTypeHl"

        exe "hi! link @lsp.type.enumMember.cpp NeosolarizedIdentifier"
endif
" }}}
