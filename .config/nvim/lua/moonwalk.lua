...
local log = (function()
    local logpath = vim.fs.normalize(cachedir .. "/moonwalk.log")
...
local function compile(path)
    ...
    local lua_filepath = path:gsub("%." .. ext .. "$", ".lua")
    if vim.fn.has("win32") == 1 then
        lua_filepath = "/" .. string.gsub(lua_filepath, ":", "")
    end
    local luapath = vim.fs.normalize(cachedir .. "/moonwalk" .. lua_filepath)
