-- fsm.lua
--
-- fsm.lua is a minimal file explorer for neovim. It is based on the neofs
-- plugin.
--
-- REFERENCE:
...
function M.util.fs_readdir(dir)
  local output = vim.fn.readdir(dir)
  local result = vim.tbl_map(function (name)
...
local function display_items(buffer, items)
  vim.api.nvim_buf_call(buffer, function()
    ...
    vim.api.nvim_buf_set_lines(buffer, 0, -1, false, vim.tbl_map(item_to_display_text, items))
